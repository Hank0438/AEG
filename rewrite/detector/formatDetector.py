...
class printFormat(angr.SimProcedure):
    IS_FUNCTION = True
    '''
    ...
    def checkExploitable(self):
        ...
        for i in range(5):
            ...
            var_loc = state_copy.solver.eval(printf_arg)
            ...
            if greatest_count > 0:
                ...
                vuln_string = state_copy.solver.eval(var_value, cast_to=bytes)
...
def checkFormat(binary_name,inputType="STDIN"):
    ...
    p.hook(addr=printf_plt, hook=printFormat())
